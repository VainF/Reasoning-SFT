...
class Encoder(object):
    ...
    def encode(self, datas):
        ...
        is_truncated = False
        global N_PRINT, TOTAL_PRINT
        for data in datas:
            text = [
                {'role': 'user', 'content': data["instruction"]},
                {'role': 'assistant', 'content': data['output']}
            ]
            input_ids = self.tokenizer._tokenizer.apply_chat_template(text[:-1])
            if len(input_ids) >= self.seq_length:
                print(f"Extreme long user input (len={len(input_ids)}), omitted...")
                continue
                        
            all_ids = self.tokenizer._tokenizer.apply_chat_template(text)
            if len(all_ids) >= self.seq_length:
                #print(f"Extreme long sequence (len={len(all_ids)}), truncted...")
            ...
            y_ids = [IGNORED_INDEX] * (len(input_ids) - 1) + all_ids[len(input_ids):] + [IGNORED_INDEX]

            if N_PRINT<TOTAL_PRINT:
                print(self.tokenizer._tokenizer.apply_chat_template(text,tokenize=False))
                print("Input:", all_ids)
                print("Label:", y_ids)
                N_PRINT += 1
                # flush
            ...
            if sum(sentence_lens) + len(all_ids) > self.seq_length:
                if self.seq_length > sum(sentence_lens):
                    ...
                    label_ids = label_ids + [IGNORED_INDEX] * (self.seq_length - sum(sentence_lens))     
        ...
        if sum(sentence_lens) > 0:
            ...
            ids['text'] = doc_ids + label_ids
            lens['text'] = [len(doc_ids) * 2]
        yield ids, lens, len(json.dumps(ids)), is_truncated
...
class Partition(object):
    ...
    def print_processing_stats(self, count, proc_start, total_bytes_processed, n_truncated):
        if count % self.args.log_interval == 0:
            current = time.time()
            elapsed = current - proc_start
            mbs = total_bytes_processed/elapsed/1024/1024
            print(f"Processed {count} documents",
                  f"({count/elapsed} docs/s, {mbs} MB/s). {n_truncated} truncated samples.",
                  file=sys.stderr)
    ...
    def process_json_file(self, file_name):
        ...
        for datas in encoded_docs:
            for (doc, sentence_lens, bytes_processed, is_truncated) in datas:
                ...
                if is_truncated:
                    n_truncated += 1
                cnt+=1
                self.print_processing_stats(cnt, proc_start, total_bytes_processed, n_truncated)
                
        print(f"After pre-tokenizing, the idxmap dataset has {cnt - 1} samples, {n_truncated} samples are truncated.")
