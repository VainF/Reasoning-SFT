...
class GPTDataset(MegatronDataset):
    ...
    def __init__(
        ...
        self.sft = args.sft
        self.actual_seq_length = args.seq_length
    ...
    def __getitem__(self, idx: Optional[int]) -> Dict[str, torch.Tensor]:
        ...
        if self.sft:
            tokens_and_labels = text
            actual_seq_length = self.actual_seq_length
            tokens_and_labels = tokens_and_labels.long()
            tokens = tokens_and_labels[:actual_seq_length]
            labels = tokens_and_labels[actual_seq_length:]
        elif self.config.add_extra_token_to_sequence:
        ...
        #print(f"init loss mask: {loss_mask.sum()}")
        # For padded sequences, mask the loss
        if self.sft:
            loss_mask = loss_mask * (labels != _IGNORED_INDEX) # mask the loss for the ignored index
        else:
            loss_mask = loss_mask * (labels != self._pad_token_id)

        #print(f"tokens: {tokens.detach().cpu().tolist()}")
        #print(f"labels: {labels.detach().cpu().tolist()}")
        #print(f"loss_mask: {loss_mask.sum()}")
